...
function PauseMenu(level, game_numbers, game_state) {
  ...
  this.buttons.push(new ImpulseButton("QUIT", 25, this.x + this.w/4, this.y + this.h/2 - 20, 200, 50, function(_this) { return function() {
          if(_this.game_state.world_num) {
            switch_game_state(new ClassicSelectState(_this.game_state.world_num))
          }
          else {
  ...
  this.buttons.push(new ImpulseButton("RESTART", 25, this.x - this.w/4, this.y + this.h/2 - 20, 200, 50, function(_this) { return function() {
...
PauseMenu.prototype.additional_draw = function(ctx) {
  ...
  for(var i = 0; i < 3; i++) {
    ...
    ctx.fillText(impulse_level_data[this.level_name].cutoff_scores[i], this.x, this.y - this.h/2 + 170 + 30 * i)
  }
  ctx.fill()
