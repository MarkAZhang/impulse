var Stunner = function(world, x, y, id) {
  this.init(world, x, y, id)
}

//the dampening factor that determines how much "air resistance" unit has
Stunner.prototype.lin_damp = 2.99

//how fast enemies move
Stunner.prototype.force = .5

//how fast enemies move when cautious
Stunner.prototype.slow_force = .1

//how often enemy path_finds
Stunner.prototype.pathfinding_delay = 100

//how often enemy checks to see if it can move if yielding
Stunner.prototype.yield_delay = 10

Stunner.prototype.init = function(world, x, y, id) {
...
Stunner.prototype.process = function(enemy_index) {
...
Stunner.prototype.start_death = function(death) {
  this.dying_start = (new Date()).getTime()
  this.dying = death
}

Stunner.prototype.collide_with = function(player) {
...
Stunner.prototype.draw = function(context, draw_factor) {
  if(this.dying) {
    var prog = ((new Date()).getTime() - this.dying_start) / 500
    context.beginPath()
    context.strokeStyle = 'rgba(255, 0, 0, '+(1-prog) + ')';
