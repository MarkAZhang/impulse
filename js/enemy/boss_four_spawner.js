...
BossFourSpawner.prototype.additional_processing = function(dt) {
  ...
  if(this.spawned) {
    if(this.spawn_action_timer > 0) {
      ...
      this.set_size(impulse_enemy_stats[this.type].effective_radius * cur_factor)
      this.spawn_action_timer -= dt
    } else {
      if(this.size != impulse_enemy_stats[this.type].effective_radius) {
        this.set_size(impulse_enemy_stats[this.type].effective_radius)
      }
    }  
  }
...
BossFourSpawner.prototype.draw  = function(context, draw_factor) {
  ...
  context.lineWidth = 2
  context.strokeStyle = impulse_enemy_stats[this.enemy_type].color
  context.stroke()*/
  context.restore()
