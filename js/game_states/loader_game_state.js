  ...
  visibility_graph_worker.onmessage = function(_this) {
    return function(event) {
      if(event.data.print) {
        ...
        _this.visibility_graph = new VisibilityGraph(level.boundary_polygons, level, event.data.poly_edges, event.data.vertices, event.data.edges, event.data.edge_list, event.data.shortest_paths)
